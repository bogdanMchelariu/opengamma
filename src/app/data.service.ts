...
export class DataService {
	...
	submitCalculation(): Observable<string> {
		const endTime = Date.now() + (MIN_TIME + Math.random() * (MAX_TIME - MIN_TIME));
		const id = this.lastId++;
		this.endTimeById[id] = endTime;
		console.log('POST - submitCalculation for id: ', id);
	...
	requestResult(id: string): Observable<Result> {
		console.log('GET - requestResult for: ', id);
	...
	private createImGroupings(calculationId: string, level = 0): ImGrouping {
		...
		for (let i = 0; i < numberOfChildren; i++) {
			const name = `Trader${calculationId}`;
