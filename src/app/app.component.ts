...
import { map, mergeMap, scan, share, startWith, tap, takeUntil, takeWhile } from 'rxjs/operators';
...
export class AppComponent implements OnInit {
	...
	ngOnInit(): void {
		this.completedValues$ = observableRange(1, this.totalParts).pipe(
			...
			share(),
			tap(res =>
				res.length === this.totalParts
					? setTimeout(() => this.calculationService.stopCalculateSource.next(false))
					: null
			)
		);
